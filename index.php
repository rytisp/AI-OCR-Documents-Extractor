<?php
  ...
  define('DB_NAME', 'ai');
  define('DB_USER', 'root');
  define('DB_PASS', 'foM,@i!6/7wOnP(7T0hiL3~wV7=');
    ...
    foreach ($images as $img) {
      ...
      if (trim($pageText) === '') {
        ...
        $payload = [
          ...
          "messages" => [[
            "role"    => "user",
            "content" => "Extract all visible Portuguese text.",
            "images"  => [$b64]
          ]],
  ...
  function call_ollama_extraction($text) {
    $prompt = "Extract structured info from the OCR text for fields like name, address, NIF, etc. Return valid JSON with all possible values, missing as null.\n\nOCR TEXT:\n$text";
    $payload = [
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
      ...
      foreach ($fields as $f) if (!array_key_exists($f, $allResults)) $allResults[$f] = null;
      $colsSQL = implode(',', array_map(fn($f) => "`$f`", $fields));
      $placeholders = implode(',', array_map(fn($f) => ":$f", $fields));
      $sql = "INSERT INTO extractions ($colsSQL) VALUES ($placeholders)";
      $stmt = $pdo->prepare($sql);
      foreach ($fields as $f) $stmt->bindValue(":$f", $allResults[$f] ?? null);
      $stmt->execute();
      echo json_encode(['ok' => true, 'data' => $allResults, 'insert_id' => $pdo->lastInsertId()]);
    } catch (Throwable $e) {
